...
let state = { lists: [], categories: [], shopping: [], backlog: [], calendar: [], recipes: [], notes: [] };
let noteQuery = "";
...
const VIEW_PARENT = { lists: "dashboard", listdetail: "lists", calendar: "dashboard", backlog: "dashboard", notes: "dashboard", guide: "dashboard" };
...
async function remoteFetchAll() {
  const [lists, cats, shop, backlog, cal, recipes, notes] = await Promise.all([
  ...
  const err = lists.error || cats.error || shop.error || backlog.error || cal.error || recipes.error || notes.error;
  if (err) throw err;
...
document.addEventListener("DOMContentLoaded", init);  state.shopping = shop.data || [];
  state.backlog = backlog.data || [];
  state.calendar = cal.data || [];
  state.recipes = (recipes.data || []).map(r => ({ ...r, ingredients: r.ingredients || [] }));
}
