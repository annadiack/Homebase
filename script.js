...
document.addEventListener("DOMContentLoaded", init);function localLoad() {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) { state = JSON.parse(raw); }
  } catch (e) { /* ignorieren */ }
...
(function initAngebotSearch() {
  ...
  const go = () => {
    const q = input.value.trim();
    if (!q) return;
    const base = prospekteLand === "ch" ? "https://www.marktguru.ch/suche/" : "https://www.marktguru.de/suche/";
